...
open Contracts.UniswapV2Router.ContractDefinition
    ...
    type UniswapV2RouterService (web3: Web3, contractAddress: string) =
    
        member val Web3 = web3 with get
        member val ContractHandler = web3.Eth.GetContractHandler(contractAddress) with get
    
        static member DeployContractAndWaitForReceiptAsync(web3: Web3, uniswapV2RouterDeployment: UniswapV2RouterDeployment, ?cancellationTokenSource : CancellationTokenSource): Task<TransactionReceipt> = 
            let cancellationTokenSourceVal = defaultArg cancellationTokenSource null
            web3.Eth.GetContractDeploymentHandler<UniswapV2RouterDeployment>().SendRequestAndWaitForReceiptAsync(uniswapV2RouterDeployment, cancellationTokenSourceVal)
        
        static member DeployContractAsync(web3: Web3, uniswapV2RouterDeployment: UniswapV2RouterDeployment): Task<string> =
            web3.Eth.GetContractDeploymentHandler<UniswapV2RouterDeployment>().SendRequestAsync(uniswapV2RouterDeployment)
        
        static member DeployContractAndGetServiceAsync(web3: Web3, uniswapV2RouterDeployment: UniswapV2RouterDeployment, ?cancellationTokenSource : CancellationTokenSource) = async {
            let cancellationTokenSourceVal = defaultArg cancellationTokenSource null
            let! receipt = UniswapV2RouterService.DeployContractAndWaitForReceiptAsync(web3, uniswapV2RouterDeployment, cancellationTokenSourceVal) |> Async.AwaitTask
            return new UniswapV2RouterService(web3, receipt.ContractAddress);
            }
